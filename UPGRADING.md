...
- [BC] The `self-accessing-callable`, `self-accessing-Closure`, `self-mutating-callable` and `self-mutating-Closure` types were removed: use `Closure<read-props>(...)` and `Closure<write-this-props|write-refs>(...)` (or any other capability set). `pure`, `impure`, `read-props`, `write-this-props`, `write-props`, `read-globals`, `write-globals`, `write-refs` and `io` are now reserved type names.
- [BC] Capability sets are printed without the `mutation-free` and `external-mutation-free` aliases (`read-props`, `write-this-props|write-refs`), and `MissingPureAnnotation`, `MissingImmutableAnnotation`, `MissingAbstractPureAnnotation`, `MissingInterfaceImmutableAnnotation` and their `--alter` fixes use `@psalm-capabilities` instead of `@psalm-mutation-free` and `@psalm-external-mutation-free` (`@psalm-pure`, `@psalm-impure`, `@psalm-immutable` and `@psalm-mutable` are still suggested). `@psalm-mutation-free` and `@psalm-external-mutation-free` are still accepted but no longer documented; `mutation-free` and `external-mutation-free` are not capability names (use `read-props` and `write-this-props|write-refs` in `@psalm-capabilities`, closure types and purity template arguments). `@psalm-capabilities` on a class needing at most `read-props` now specializes taints like `@psalm-pure` and `@psalm-mutation-free` do.
- [BC] Calling a method that writes the properties of its `$this` costs the caller `write-this-props` when the receiver is the caller's `$this`, `write-props` when it is any other object, and nothing when it is an object the caller created from a class needing at most `write-this-props|write-refs`. Previously any receiver of the caller's own class, and `$this` in a `@psalm-mutation-free` method, were free, and other receivers only cost `write-this-props`.
...
- [BC] `clone` (`__clone`), string interpolation and `(string)` casts (`__toString`), `$object()` (`__invoke`), `ArrayAccess` and `__get` calls (`isset()` and `unset()` on an `ArrayAccess` object call `offsetExists`/`offsetUnset`), `throw new`, `new $className` (an unknown class counts as impure), parameter default values, literal callable strings and `call_user_func` are now checked for purity; unknown callable strings and arrays count as impure. `Exception::__construct` and `Error::__construct` need `write-this-props|write-refs`.
- [BC] Builtin functions with side effects now require specific capabilities: `io` for I/O, the clock and the CSPRNG (`time`, `file_put_contents`, `random_int`, …), `write-globals` for those touching process-wide state (`mt_rand`, `rand`, `ini_set`, `spl_autoload_register`, …), `write-props` for `hash_update`; `call_user_func`, `class_exists`, `unserialize` and the iterator helpers stay impure. First-class callables of builtin functions (`mt_rand(...)`) carry those capabilities.
- [BC] Values reached from global state (static properties, superglobals, `global` variables, the results of callees that may read globals, and anything fetched from those, tracked by `Psalm\Type\Union#$from_global_state`) can only have their properties written, their mutating methods called, or be passed to a callee that may write properties, with `write-globals`.
- [BC] A closure capturing a variable by reference (`use (&$x)`) needs `read-props` to read it and `write-this-props` to write it, instead of being impure; its enclosing scope may call it freely, and needs `write-refs`/`write-globals` only when the captured variable is a by-reference parameter or a global (`Psalm\Context#$captured_by_ref`). Writing a by-reference parameter, or any reference into another scope, needs `write-refs` only, and a function with `write-refs` may return `void`.
- [BC] Calling a function-like that may write its by-reference parameters (`write-refs`, including builtins such as `sort`) costs what the arguments passed by reference are, not `write-refs` itself: nothing for a local, `write-refs` for a by-reference parameter, `write-props`/`write-this-props` for a property, `write-globals` for global state. So `@psalm-capabilities write-props` code may call `@psalm-external-mutation-free` methods.
...
- `reset()`, `end()`, `next()` and `prev()` are pure: moving an array's internal pointer is not an effect, even though they take the array by reference.
...
- [BC] `Traversable`, `Iterator`, `IteratorAggregate`, `OuterIterator`, `RecursiveIterator`, `SeekableIterator` and `Generator` have a new last template, `TPurity` (default `impure`), what iterating over the object may do; the `Iterator` and `IteratorAggregate` methods need `read-props` plus `TPurity` and the `Generator` methods only `TPurity` (a generator's position is not an effect), and `Generator` now implements `Iterator`. A generator function-like with a purity annotation binds `TPurity` in its return type to its own capabilities and purity templates; a class implementing `Iterator`/`IteratorAggregate` binds it in `@implements` or gets it from its iteration methods, a method writing the iterator's own properties counting as `write-props` (the override check of those methods then uses the binding). `foreach` over an object costs the `Iterator` methods it calls (or `getIterator()` and the iterator it returns, treated as fresh) instead of always being impure; iterating a value known only as `Traversable`/`Iterator`/`IteratorAggregate`/`Generator` costs its `TPurity` alone, and iterating a value of an iterator class costs its methods like any method call (writing the iterator is free when it is freshly created, by `new` or a call of a generator function-like, and costs `write-this-props` when it is `$this` and `write-props` otherwise). Inherited iterator types show the template (`Traversable<int, Foo, impure>`). Trailing purity templates with a default may be left out of `@extends`/`@implements` and of docblock types, and a left-out one counts as its default in comparisons.
- [BC] Destroying an object calls its `__destruct`: a function-like that keeps, in a local variable, an object it created with `new` and never lets escape, that `unset()`s a variable holding an object, or that discards a `new` expression, now needs the destructor's capabilities (what the destructor does to the destroyed object itself is free).
- [BC] Parameter default values of a function-like with a purity annotation are evaluated with no capabilities, or with `read-globals|write-globals` when the function-like has `write-globals`, like in Hack; previously they were evaluated with the function-like's own capabilities. Function-likes without a purity annotation are not affected. `Psalm\Context#$inside_param_default` tells apart such analysis.
- With `rememberPropertyAssignmentsAfterCall="false"`, property refinements now survive `new` and static calls of callees that cannot write properties, and static property refinements survive callees that cannot write globals (previously every `new`/static call, and only callees writing properties, forgot them).
- `@psalm-purity-template` accepts bounds written as a chain, `lower <= Name(default) <= upper`: an upper bound (`P <= write-props`) and, on classes, a lower bound (`write-this-props <= C`, which the methods depending on `C` may use unconditionally) and a default (`C(pure) <= write-props`); `Closure<_>(...)` in a parameter's type declares an implicit purity template; `@psalm-capabilities` accepts type aliases (`@psalm-type`, `@psalm-import-type`). An unknown capability name is now reported as `InvalidDocblock` rather than `MissingDocblockType`.
