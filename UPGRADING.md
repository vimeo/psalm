...
- [BC] A closure capturing a variable by reference (`use (&$x)`) needs `read-props` to read it and `write-this-props` to write it, instead of being impure; its enclosing scope may call it freely, and needs `write-refs`/`write-globals` only when the captured variable is a by-reference parameter or a global (`Psalm\Context#$captured_by_ref`). Writing a by-reference parameter needs `write-refs` only, and a function with `write-refs` may return `void`.
