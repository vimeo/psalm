...
- [BC] `Traversable`, `Iterator`, `IteratorAggregate`, `OuterIterator`, `RecursiveIterator`, `SeekableIterator` and `Generator` have a new last template, `TPurity` (default `impure`), what iterating over the object may do; the `Iterator` methods and `Generator` methods are `@psalm-external-mutation-free` plus `TPurity`, `IteratorAggregate::getIterator()` `@psalm-mutation-free` plus `TPurity`, and `Generator` now implements `Iterator`. A generator function-like with a purity annotation binds `TPurity` in its return type to its own capabilities and purity templates; a class implementing `Iterator`/`IteratorAggregate` binds it in `@implements` or gets it from its iteration methods (the override check of those methods then uses the binding). `foreach` over an object costs the `Iterator` methods it calls (or `getIterator()` and the iterator it returns, treated as fresh) instead of always being impure; the iterator's own state is free to touch when it is `$this` or freshly created (`new`, or a call of a generator function-like). Inherited iterator types show the template (`Traversable<int, Foo, impure>`). Trailing purity templates with a default may be left out of `@extends`/`@implements` and of docblock types, and a left-out one counts as its default in comparisons.
