...
- [BC] The `self-accessing-callable`, `self-accessing-Closure`, `self-mutating-callable` and `self-mutating-Closure` types were removed: use `Closure<read-props>(...)` and `Closure<write-this-props|write-refs>(...)` (or any other capability set). `pure`, `impure`, `mutation-free`, `external-mutation-free`, `read-props`, `write-this-props`, `write-props`, `read-globals`, `write-globals`, `write-refs` and `io` are now reserved type names.
- [BC] Capability sets are printed without the `mutation-free` and `external-mutation-free` aliases (`read-props`, `write-this-props|write-refs`), and `MissingPureAnnotation`, `MissingImmutableAnnotation`, `MissingAbstractPureAnnotation`, `MissingInterfaceImmutableAnnotation` and their `--alter` fixes use `@psalm-capabilities` instead of `@psalm-mutation-free` and `@psalm-external-mutation-free` (`@psalm-pure`, `@psalm-impure`, `@psalm-immutable` and `@psalm-mutable` are still suggested). `@psalm-mutation-free` and `@psalm-external-mutation-free`, and the aliases in `@psalm-capabilities` and closure types, are still accepted but no longer documented. `@psalm-capabilities` on a class needing at most `read-props` now specializes taints like `@psalm-pure` and `@psalm-mutation-free` do.
...
- [BC] Builtin functions with side effects now require specific capabilities: `io` for I/O, the clock and the CSPRNG (`time`, `file_put_contents`, `random_int`, …), `write-globals` for those touching process-wide state (`mt_rand`, `rand`, `ini_set`, `spl_autoload_register`, …), `write-props` for `hash_update`; `call_user_func`, `class_exists`, `unserialize` and the iterator helpers stay impure. `dictionaries/ImpureFunctionsList.php` now maps each function to its capabilities, and `Psalm\Internal\Codebase\ImpureFunctionsList::getCapabilities()` returns them. First-class callables of builtin functions (`mt_rand(...)`) carry those capabilities.
...
- [BC] `Traversable`, `Iterator`, `IteratorAggregate`, `OuterIterator`, `RecursiveIterator`, `SeekableIterator` and `Generator` have a new last template, `TPurity` (default `impure`), what iterating over the object may do; the `Iterator` methods need `write-this-props|write-refs` plus `TPurity` and the `Generator` methods only `TPurity` (a generator's position is not an effect), `IteratorAggregate::getIterator()` `read-props` plus `TPurity`, and `Generator` now implements `Iterator`. A generator function-like with a purity annotation binds `TPurity` in its return type to its own capabilities and purity templates; a class implementing `Iterator`/`IteratorAggregate` binds it in `@implements` or gets it from its iteration methods (the override check of those methods then uses the binding). `foreach` over an object costs the `Iterator` methods it calls (or `getIterator()` and the iterator it returns, treated as fresh) instead of always being impure; moving the iterator along is free when it is freshly created (`new`, or a call of a generator function-like), and costs `write-this-props` when it is `$this` and `write-props` otherwise. Inherited iterator types show the template (`Traversable<int, Foo, impure>`). Trailing purity templates with a default may be left out of `@extends`/`@implements` and of docblock types, and a left-out one counts as its default in comparisons.
...
- With `rememberPropertyAssignmentsAfterCall="false"`, property refinements now survive `new` and static calls of callees that cannot write properties, and static property refinements survive callees that cannot write globals (previously every `new`/static call, and only callees writing properties, forgot them). `Psalm\Context#removeMutableObjectVars()` takes the callee's capabilities as a second argument.
