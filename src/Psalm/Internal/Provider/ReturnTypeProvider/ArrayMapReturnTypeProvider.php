        ...
        if ($function_call_type && $function_call_type->isNull()) {
            ...
            foreach ($call_args as $call_arg) {
                ...
                if ($call_arg_type
                    ...
                    && $call_arg_atomic->fallback_value_type === null
                ) {
            ...
            $array_arg_types = array_map(
                ...
                function (array $sub) use ($null) {
                    ...
                    return new Union([new TKeyedArray($sub, null, null, null, true)]);
                },
                $array_arg_types
            );
            assert(count($array_arg_types));

            return new Union([new TKeyedArray($array_arg_types, null, null, null, true)]);
        }
        ...
        if ($mapping_return_type && $generic_key_type) {
            if ($array_arg_atomic_type instanceof TKeyedArray && count($call_args) === 2) {
                $atomic_type = new TKeyedArray(
                    ...
                    null,
                    $array_arg_atomic_type->fallback_key_type,
                    $array_arg_atomic_type->fallback_value_type === null
                        ? null
                        : $mapping_return_type,
