...
class AssignmentChecker
{
    ...
    public static function check(
        ...
        $type_in_comments = CommentChecker::getTypeFromComment($doc_comment, $context, $statements_checker->getSource(), $var_id);
        ...
        else if ($assign_var instanceof PhpParser\Node\Expr\ArrayDimFetch) {
            if (self::checkArrayAssignment($statements_checker, $assign_var, $context, $return_type) === false) {
                return false;
            }

        }
        else if ($assign_var instanceof PhpParser\Node\Expr\PropertyFetch &&
        ...
        else if ($assign_var instanceof PhpParser\Node\Expr\StaticPropertyFetch &&
        ...
        if ($var_id && isset($context->vars_in_scope[$var_id]) && $context->vars_in_scope[$var_id]->isVoid()) {
            if (IssueBuffer::accepts(
                new FailedTypeResolution('Cannot assign ' . $var_id . ' to type void', $statements_checker->getCheckedFileName(), $assign_var->getLine()),
