...
/**
 * Denotes the `interface-string` type, used to describe a string representing a valid PHP class.
 * The parent type from which the classes descend may or may not be specified in the constructor.
 */
class TInterfaceString extends TString
{
    /**
     * @var string
     */
    ...
    {
        return 'interface-string' . ($this->as === 'object' ? '' : '<' . $this->as_type . '>');
    }
    ...
    public function toNamespacedString(
        ...
        if ($namespace && stripos($this->as, $namespace . '\\') === 0) {
            return 'interface-string<' . preg_replace(
        ...
        if (!$namespace && strpos($this->as, '\\') === false) {
            return 'interface-string<' . $this->as . '>';
        }

        if (isset($aliased_classes[strtolower($this->as)])) {
            return 'interface-string<' . $aliased_classes[strtolower($this->as)] . '>';
        }
    ...
    public function replaceTemplateTypesWithStandins(
        ...
        } elseif ($input_type instanceof TInterfaceString && $input_type->as_type) {
